...
const AVALIADORA = (() => {
  ...
  function calcImpostos(fatMensal, regime, setorCode, fatAnual) {
    ...
    if (regime === 'lucro_presumido' || regime === 'presumido') {
      ...
      const aliq = tipoServico ? 0.1133 : 0.0593;
      return { mensal: fat * aliq, pct: aliq * 100, regime: 'Lucro Presumido', detalhes: tipoServico ? '11,33% (serviços)' : '5,93% (comércio)' };
    }

    if (regime === 'lucro_real') {
      // PIS + COFINS sobre faturamento
      const tribFat = fat * 0.0925;
      // IRPJ + CSLL calculado sobre RO (passado como parâmetro externo — aqui estimativa)
      return { mensal: tribFat, pct: 9.25, regime: 'Lucro Real', detalhes: 'PIS 1,65% + COFINS 7,6% (IRPJ/CSLL calculado sobre RO)', semIrpj: true };
    }
  ...
  function mapDados(dados) {
    ...
    const fatAnual = n(p1(d.fat_anual, dados.faturamento_anual, fat * 12));
    const regime = (d.regime || d.regime_tributario || dados.regime_tributario || 'simples').toLowerCase().replace(' ','_');
    ...
    return {
      ...
      anos: n(p1(d.anos_existencia, d.cnpj_anos, dados.anos_existencia)),
      regime,
      fat,
      fatAnual,
      ...
      comissoes: n(d.custo_comissoes) > 0 ? n(d.custo_comissoes)
        : ((n(d.pct_comissao) || n(d.comissao_pct)) > 0 ? fat * (n(d.pct_comissao) || n(d.comissao_pct)) / 100 : 0),
      ...
      outros_cf: n(p1(d.outros_cf, d.outros_custos_fixos)),
      ...
      crescimento: d.crescimento || 'estavel',
  ...
  function calcDRE(D, setorCode) {
    ...
    // IMPOSTOS — usar pré-calculado do diagnóstico se disponível e consistente
    let impostos;
    if (D.impostos_precalc > 0) {
      ...
      const imp = calcImpostos(fat, D.regime, setorCode, D.fatAnual);
      impostos = imp.mensal;
    }
    ...
    // FOLHA TOTAL
    const folha = D.folha_total > 0 ? D.folha_total : cltFolha + encargos + provisoes + D.pj_custo;

    // DRE
    const recLiq = fat - impostos - D.taxas - D.comissoes - D.royalty - D.mkt_franq;
    const lb = recLiq - D.cmv;
    const ro = lb - folha - D.aluguel - D.facilities - D.terceirizados - D.outros_cf - D.mkt;
    ...
    return {
      ...
      recLiq, cmv: D.cmv, lb,
      clt_folha: cltFolha, encargos, provisoes, pj_custo: D.pj_custo, folha,
      aluguel: D.aluguel, facilities: D.facilities,
      terceirizados: D.terceirizados, outros_cf: D.outros_cf, mkt: D.mkt,
  ...
  function calcAtratividade(D, dre, ise, setorCode) {
    ...
    const crescMap = { 'mais20': 9, 'ate20': 7, 'ate10': 5, 'estavel': 4, 'caindo': 2 };
  ...
  function calcICD(D, dre) {
    const checks = {
      fat_mensal:          D.fat > 0,
      regime:              !!D.regime,
      meios_recebimento:   D._raw && D._raw.meios_selecionados && D._raw.meios_selecionados.length > 0,
      cmv:                 D.cmv_pct > 0 || D.cmv > 0,
      folha_clt:           D.clt_folha > 0 || D.folha_total > 0,
      aluguel:             D.aluguel > 0,
      prolabore:           D.prolabore > 0,
      clientes_ativos:     D.clientes > 0,
      recorrencia:         D._raw && D._raw.recorrencia_pct !== undefined && D._raw.recorrencia_pct !== null,
      processos:           !!D.processos && D.processos !== '',
      saldo_devedor:       D._raw && D._raw.saldo_devedor !== undefined,
    };

    const respondidos = Object.keys(checks).filter(k => checks[k]);
    const naoRespondidos = Object.keys(checks).filter(k => !checks[k]);
    return {
      pct: Math.round(respondidos.length / Object.keys(checks).length * 100),
      respondidos,
      naoRespondidos,
    };
  }
  ...
  function calcRegimes(D, dre, setorCode) {
    ...
    // Lucro Real — PIS/COFINS sobre fat + IRPJ/CSLL sobre RO
    const tribFat = fat * 0.0925;
    const tribRO = ro > 0 ? ro * 0.34 : 0;
    const totalReal = tribFat + tribRO;
    resultados.push({ regime:'Lucro Real', elegivel:true, imposto_mensal:totalReal, pct:fat>0?totalReal/fat*100:0, detalhes:`PIS/COFINS ${brl(tribFat)} + IRPJ/CSLL ${brl(tribRO)}` });

    // Identificar regime atual e regime ótimo
    const atual = resultados.find(r => r.regime.toLowerCase().includes(regimeAtual.replace('_',' '))) || resultados.find(r=>r.elegivel);
    ...
    return {
      ...
      tem_oportunidade: economia > fat * 0.01, // só mostra se economia > 1% do fat
    };
  }

  // ─── 15. GERAR UPSIDES ──────────────────────────
  function gerarUpsides(D, dre, ise, setorCode) {
    const fat = dre.fat;
    ...
    // CALCULADAS (baseadas em desvio do benchmark)

    // CMV acima do benchmark
    const cmvPct = fat > 0 ? pct(dre.cmv, fat) : 0;
    if (cmvPct > bench.cmv) {
      const ganho = Math.round(fat * (cmvPct - bench.cmv) / 100 * 0.4);
      ops.push({ titulo:'Reduzir CMV para benchmark setorial', descricao:`CMV em ${cmvPct.toFixed(0)}% vs benchmark ${bench.cmv}%. Renegociação com fornecedores e ajuste de cardápio pode liberar resultado.`, ganho, ganho_label:`+${brl(ganho)}/mês`, tipo:'calculada' });
    }

    // Folha acima do benchmark
    const folhaPct = fat > 0 ? pct(dre.folha, fat) : 0;
    if (folhaPct > benchInd.folha_pct * 1.15) {
      const ganho = Math.round(fat * (folhaPct - benchInd.folha_pct) / 100 * 0.3);
      ops.push({ titulo:'Otimizar estrutura de equipe', descricao:`Folha representa ${folhaPct.toFixed(0)}% do faturamento vs benchmark de ${benchInd.folha_pct}%. Revisão de estrutura pode liberar resultado sem impacto operacional.`, ganho, ganho_label:`+${brl(ganho)}/mês`, tipo:'calculada' });
    }

    // Aluguel acima do benchmark
    const aluPct = fat > 0 ? pct(dre.aluguel, fat) : 0;
    if (aluPct > benchInd.aluguel_pct * 1.2 && dre.aluguel > 0) {
      const ganho = Math.round(fat * (aluPct - benchInd.aluguel_pct) / 100 * 0.5);
      ops.push({ titulo:'Renegociar contrato de aluguel', descricao:`Aluguel em ${aluPct.toFixed(0)}% do faturamento vs benchmark ${benchInd.aluguel_pct}%. Com histórico de operação, renegociação é viável.`, ganho, ganho_label:`+${brl(ganho)}/mês`, tipo:'calculada' });
    }

    // Dependência do sócio
    if (ise.dep < 6) {
      ops.push({ titulo:'Reduzir dependência do sócio-fundador', descricao:'Designar gestor autônomo para operação. Eleva ISE em 10+ pontos e melhora o Fator 1N diretamente.', ganho:0, ganho_label:'+Fator 1N', tipo:'calculada' });
    }

    // Recorrência baixa
    if (D.recorrencia_pct < 30) {
      const ganho = Math.round(fat * 0.08);
      ops.push({ titulo:'Converter clientes avulsos em recorrentes', descricao:`Recorrência atual de ${D.recorrencia_pct}%. Estruturar planos ou contratos mensais. Cada 10% de aumento eleva o múltiplo aplicável.`, ganho, ganho_label:`+${brl(ganho)}/mês`, tipo:'calculada' });
    }

    // Processos não documentados
    if (D.processos !== 'documentados') {
      ops.push({ titulo:'Documentar processos operacionais', descricao:'Criar SOPs para as principais funções. Reduz risco percebido pelo comprador e acelera due diligence.', ganho:0, ganho_label:'+ISE Gestão', tipo:'calculada' });
    }

    // Concentração alta
    if (D.concentracao_pct > 20) {
      ops.push({ titulo:'Diversificar base de clientes', descricao:`Maior cliente representa ${D.concentracao_pct}% do faturamento. Reduzir para abaixo de 20% elimina desconto de risco no Fator 1N.`, ganho:0, ganho_label:'+Fator 1N', tipo:'calculada' });
    }

    // Marca não registrada
    if (D.marca_inpi !== 'sim') {
      ops.push({ titulo:'Registrar marca no INPI', descricao:'Formalizar o registro da marca. Eleva pilar de Marca no ISE e protege o ativo intangível da empresa.', ganho:0, ganho_label:'+ISE Marca', tipo:'calculada' });
    }

    // AÇÕES FIXAS POR SETOR (completar até 10)
    const fixasPorSetor = {
      alimentacao: [
        { titulo:'Expandir canal de delivery', descricao:'Estruturar delivery próprio ou ampliar parceria com iFood/Rappi. Canal de baixo custo fixo com alto potencial de escala.', ganho: Math.round(fat*0.08), ganho_label:`+${brl(Math.round(fat*0.08))}/mês` },
        { titulo:'Criar menu executivo ou happy hour', descricao:'Operação em horários ociosos com equipe existente. Alto potencial de incremento sem custo fixo adicional.', ganho: Math.round(fat*0.06), ganho_label:`+${brl(Math.round(fat*0.06))}/mês` },
        { titulo:'Eventos privados e locação do espaço', descricao:'Jantares corporativos e celebrações em dias de menor movimento. Receita complementar com margem elevada.', ganho: Math.round(fat*0.05), ganho_label:`+${brl(Math.round(fat*0.05))}/mês` },
        { titulo:'Programa de fidelidade e reservas online', descricao:'Reduzir no-shows e aumentar retenção com sistema de reservas e programa de pontos.', ganho: Math.round(fat*0.03), ganho_label:`+${brl(Math.round(fat*0.03))}/mês` },
      ],
      saude: [
        { titulo:'Ampliar oferta de planos e pacotes', descricao:'Estruturar pacotes de procedimentos com pagamento recorrente. Aumenta previsibilidade e reduz ociosidade.', ganho: Math.round(fat*0.10), ganho_label:`+${brl(Math.round(fat*0.10))}/mês` },
        { titulo:'Expandir horário de atendimento', descricao:'Turnos vespertinos ou noturnos com equipe existente. Aumento de capacidade sem custo fixo relevante.', ganho: Math.round(fat*0.08), ganho_label:`+${brl(Math.round(fat*0.08))}/mês` },
        { titulo:'Telemedicina e atendimento remoto', descricao:'Canal digital com custo marginal baixo e alcance ampliado. Tendência consolidada pós-pandemia.', ganho: Math.round(fat*0.05), ganho_label:`+${brl(Math.round(fat*0.05))}/mês` },
      ],
      servicos_b2b: [
        { titulo:'Estruturar funil de upsell para base atual', descricao:'Oferecer serviços complementares à base de clientes existente. Crescimento de receita sem custo de aquisição.', ganho: Math.round(fat*0.10), ganho_label:`+${brl(Math.round(fat*0.10))}/mês` },
        { titulo:'Criar produto de entrada (low ticket)', descricao:'Produto acessível como porta de entrada para clientes menores. Amplia funil e gera escala.', ganho: Math.round(fat*0.08), ganho_label:`+${brl(Math.round(fat*0.08))}/mês` },
        { titulo:'Programa de indicações estruturado', descricao:'Formalizar programa de referral para clientes atuais. Canal de aquisição de baixo custo e alto LTV.', ganho: Math.round(fat*0.06), ganho_label:`+${brl(Math.round(fat*0.06))}/mês` },
      ],
      default: [
        { titulo:'Expandir base de clientes ativos', descricao:'Crescer 20% em clientes mantendo ticket médio atual via outreach estruturado e marketing digital.', ganho: Math.round(fat*0.08), ganho_label:`+${brl(Math.round(fat*0.08))}/mês` },
        { titulo:'Estruturar funil de upsell', descricao:'Oferecer planos ou serviços superiores à base atual. Crescimento de ticket sem custo de aquisição.', ganho: Math.round(fat*0.06), ganho_label:`+${brl(Math.round(fat*0.06))}/mês` },
        { titulo:'Implantar contabilidade gerencial mensal', descricao:'DRE mensal formal aumenta o ICD e torna o valuation mais defensável em due diligence.', ganho:0, ganho_label:'+ICD' },
        { titulo:'Criar plano de retenção da equipe-chave', descricao:'Acordos de continuidade pós-venda reduzem risco percebido e melhoram condições da transação.', ganho:0, ganho_label:'+Atratividade' },
      ],
    };
    ...
    for (const f of fixas) {
      ...
      const jatem = ops.some(o => o.titulo === f.titulo);
      if (!jatem) ops.push({ ...f, tipo:'fixa' });
    }

    // Ordenar: maiores ganhos financeiros primeiro, depois qualitativas
    ops.sort((a,b) => (b.ganho||0) - (a.ganho||0));
  ...
  async function salvarCalcJson(negocioId, calcJson) {
    ...
    try {
      const payload = { calc_json: calcJson, atualizado_em: new Date().toISOString() };
      const r = await fetch(SB_URL + '/rest/v1/laudos_completos?slug=eq.' + negocioId, {
        method: 'PATCH',
        headers: { ...H, 'Prefer': 'return=minimal' },
        body: JSON.stringify(payload),
      });
      if (r.status === 404 || r.status === 0 || !r.ok) {
        await fetch(SB_URL + '/rest/v1/laudos_completos', {
          method: 'POST',
          headers: { ...H, 'Prefer': 'return=minimal' },
          body: JSON.stringify({ slug: negocioId, ...payload, pago: false }),
        });
      }
  ...
  async function avaliar(dadosBrutos) {
    ...
    const upsides = gerarUpsides(D, dre, ise, setorCode);
