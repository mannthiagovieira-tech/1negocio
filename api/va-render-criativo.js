// /api/va-render-criativo · Slice P5 · Vercel Node runtime (satori + @resvg/resvg-js)
// Substituiu @vercel/og que exigia Edge Runtime · aqui usa as MESMAS libs
// internas (satori pra SVG, resvg pra rasterizar) mas em Node · sem wasm
// loading top-level · sem drama de preflight OPTIONS 500.
//
// Renderiza 1 criativo (headline+texto+cta) em PNG no bucket criativos-png.
// 3 formatos · 3 layouts · design system 1Negócio · fontes CDN em runtime.
...
module.exports = async (req, res) => {
  // CORS + preflight primeiro · antes de qualquer coisa que possa falhar
  ...
  if (c.status === 'aprovado' && c.png_path) return json(res, 400, { ok: false, erro: 'aprovado_imutavel · desative pra re-render' });
  ...
  try {
    ...
    const svg = await satori(arv, { width: w, height: h, fonts });
