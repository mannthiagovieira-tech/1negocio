...
if(!window.AVALIADORA){ window.AVALIADORA = (() => {
  ...
  // Fórmula: Fator = (múltiplo_base + modificador_setor) × fator_ISE
  function calcFator(D, ise, setorCode) {
    ...
    // Leitura segura do múltiplo: número direto ou .mid se objeto legado
    ...
    // Fórmula limpa: (base + modificador) × ISE
    const fator = (multiploBase + modificador) * fatorIse;
    const fMin = n(lim.fator_min || 1.5);
    const fMax = n(lim.fator_max || 6.0);

    return {
      fator: Math.max(fMin, Math.min(fMax, fator)),
      modeloCode, setorCode, multiploBase, modificador,
      fatorIse, iseNome,
