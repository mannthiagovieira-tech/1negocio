...
if(!window.AVALIADORA){ window.AVALIADORA = (() => {
  ...
  function mapDados(dados) {
    ...
    return {
      ...
      outros_cf: n(p1(d.custo_outros)) + n(p1(d.custo_sistemas)),
  ...
  function calcDRE(D, setorCode) {
    ...
    const lb = recLiq - cmv;
    // outros_cf JÁ inclui facilities+terceirizados (custo_utilities+custo_terceiros+custo_outros+custo_sistemas)
  ...
  async function salvarCalcJson(negocioId, calcJson) {
    ...
    try {
      ...
      if (r.ok) {
        console.log('[AVALIADORA] calc_json salvo (upsert) para', negocioId);
      } else {
