...
const AVALIADORA = (() => {
  ...
  function mapearDados(dados) {
    ...
    return {
      ...
      custo_comissoes: n(pick(d.custo_comissoes)),

      // Franquia
      tem_royalty: d.franquia === 'sim' || dados.franquia === 'sim',
      royalty_valor: n(pick(d.royalty_valor, d.royalty_mensal, d.custo_royalties, d.royalties_valor,
        // Calcular de percentual se necessário
        (d.royalty_pct > 0 && d.fat_mensal > 0) ? d.fat_mensal * d.royalty_pct / 100 : null)),
      mkt_franquia_valor: n(pick(d.mkt_franquia_valor, d.mkt_franquia, d.fundo_valor,
        d.fundo_propaganda_valor, d.custo_fundo_mkt,
        // Calcular de percentual se necessário
        (d.mkt_franquia_pct > 0 && d.fat_mensal > 0) ? d.fat_mensal * d.mkt_franquia_pct / 100 : null)),
