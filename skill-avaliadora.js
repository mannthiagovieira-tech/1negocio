...
if(!window.AVALIADORA){ window.AVALIADORA = (() => {
  ...
  function montarCalcJson(dados, D, dre, bal, ise, fatorObj, atr, icd, upsides, regimes) {
    ...
    // ── REGRA: RO NEGATIVO ──
    // Se RO < 0, não aplica múltiplo (valuation por fluxo de caixa não faz sentido)
    // Exceção: se patrimônio líquido (ativos - passivos) > |RO anualizado|, usa PL como base
    let valorOp, valorVenda, ro_negativo_flag = false, ro_negativo_msg = '';

    if (dre.ro < 0) {
      const roAnualAbs = Math.abs(dre.ro * 12);
      const plLiquido = bal.totAtiv - bal.totPass;

      if (plLiquido > roAnualAbs) {
        // Exceção: patrimônio líquido justifica — valuation pelo PL
        valorOp = 0;
        valorVenda = plLiquido;
        ro_negativo_flag = true;
        ro_negativo_msg = 'Resultado operacional negativo. Avaliação baseada no patrimônio líquido (ativos - passivos).';
      } else {
        // RO negativo e PL não compensa — recomenda consultor
        valorOp = 0;
        valorVenda = 0;
        ro_negativo_flag = true;
        ro_negativo_msg = 'Resultado operacional negativo. Recomendamos uma sessão com consultor para avaliar seu negócio e identificar oportunidades de melhoria antes de definir um valor de venda.';
      }
    } else {
      // Fluxo normal: aplica múltiplo
      valorOp = dre.ro * 12 * fatorObj.fator;
      valorVenda = valorOp + Math.max(0, bal.pl);
    }
    ...
    return {
      ...
      ro_negativo: ro_negativo_flag,
      ro_negativo_msg: ro_negativo_msg,
