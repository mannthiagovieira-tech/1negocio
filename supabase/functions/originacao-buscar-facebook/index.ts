// originacao-buscar-facebook · v9.33.6
// Apify apify/facebook-pages-scraper · 1 query por arquétipo · escreve em pool_contatos_global+uso.
// Mesmo padrão do originacao-buscar-gmaps: paralelo · retry timeout · try/finally raiz.
//
...
function pickIdentificador(page: any): string {
  return String(
    page.facebookId || page.pageId || page.pageUrl || page.url || page.name || "",
  ).slice(0, 200);
}
...
async function processarArquetipo(
  ...
  try {
    const q = (arq.queries_busca?.fb_keywords || "").toString().trim();
    if (!q) return { ...baseStats, erro: "sem_fb_keywords" };
    baseStats.fb_keywords = q;
    ...
    if (!r.ok && r.erro && r.erro.startsWith("apify_timeout")) {
      console.log(`[orig-fb] timeout em "${arq.nome}" · retry com limite=${limiteFallback}`);
      ...
      if (r2.ok) { retryInfo = `retry OK com limite=${limiteFallback}`; r = r2; }
        ...
        return { ...baseStats, erro: "apify_timeout_persistente", retry_info: `2 tentativas timeout` };
      } else { return { ...baseStats, erro: r2.erro, retry_info: `retry após timeout falhou` }; }
    }
    ...
    for (const page of items) {
      const identificador = pickIdentificador(page);
      if (!identificador) continue;
      const nome = page.title || page.pageName || page.name || "(sem nome)";
      const telefone = page.phone || page.phoneNumber || null;
      const email = page.email || null;
      const website = page.website || page.pageUrl || page.url || null;
      const endereco = page.address || page.location || null;
      const categoria = page.categories?.[0] || page.category || null;

      const { data: upserted, error: errUp } = await adminClient
        .from("pool_contatos_global")
        .upsert({
          identificador_canonico: identificador,
          fonte_origem: "apify_facebook",
          nome,
          telefone,
          email,
          website,
          endereco_completo: typeof endereco === "string" ? endereco : null,
      ...
      if (errUp || !upserted) {
        console.error("[orig-fb] upsert err", errUp?.message);
        continue;
      }
      ...
      if (errUso) {
        ...
        else console.error("[orig-fb] insert uso err", errUso.message);
      } else inseridos++;
    }
...
serve(async (req) => {
  ...
  try {
    ...
    console.error("[orig-fb] exception raiz", e);
    if (lockSetado) {
      try { await adminClient.from("projetos_originacao").update({ leads_executando_em: null }).eq("id", originacao_id); } catch (_) {}
    }
