// originacao-buscar-gmaps · v9.33.3
// Roda Apify compass/crawler-google-places por arquétipo aprovado.
...
function pickIdentificador(place: any): string {
  // place_id é o canônico do GMaps · fallback maps_url · fallback title+address
...
async function buscarArquetipoApify(
  arq: any,
  gmapsQuery: string,
  limite: number,
...
serve(async (req) => {
  ...
  const { data: orig, error: errOrig } = await adminClient
    .from("projetos_originacao").select("id, projeto_id, fase_atual")
  ...
  // Busca arquétipos · aprovados E com queries_busca preenchidas
  ...
  for (const arq of arquetipos) {
    const gmapsQuery = (arq.queries_busca as any)?.gmaps_query;
    if (!gmapsQuery) {
      porArquetipo.push({
        arquetipo_id: arq.id, nome: arq.nome || "(sem nome)",
        gmaps_query: "", inseridos: 0, duplicados: 0, total_retornado: 0,
        erro: "queries_busca.gmaps_query vazio",
      });
      continue;
    }

    const r = await buscarArquetipoApify(arq, gmapsQuery, limiteN);
    if (!r.ok) {
      porArquetipo.push({
        arquetipo_id: arq.id, nome: arq.nome || "(sem nome)",
        gmaps_query: gmapsQuery, inseridos: 0, duplicados: 0, total_retornado: 0,
        erro: r.erro,
      });
      if (r.erro === "apify_token_invalido" || r.erro === "apify_rate_limit") {
        abortado = r.erro;
        break;
      }
      continue;
    ...
    for (const place of items) {
      ...
      if (errIns) {
        // 23505 = unique_violation → conta como duplicado
    ...
    porArquetipo.push({
      ...
      inseridos, duplicados, total_retornado: items.length,
    });
  }

  // Libera lock + marca leads_executados_em
  await adminClient
    .from("projetos_originacao")
    .update({
  ...
  const totalInseridos = porArquetipo.reduce((acc, x) => acc + x.inseridos, 0);
