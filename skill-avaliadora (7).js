...
if(!window.AVALIADORA){ window.AVALIADORA = (() => {
  ...
  function mapDados(dados) {
    ...
    return {
      ...
      anos: n(p1(dados.anos_existencia, d.anos_existencia, d.cnpj_anos)),
      regime,
      fat,
      fatAnual,
  ...
  function calcDRE(D, setorCode) {
    ...
    // CMV — usar informado ou estimativa por benchmark
    const cmv_inf = D.cmv > 0;
    ...
    // ALUGUEL — usar informado ou estimativa
    const aluguel_inf = D.aluguel > 0;
    ...
    const lb = recLiq - cmv;
    const ro = lb - folha - aluguel - facilities - terceirizados - outros_cf - mkt;
    ...
    return {
      ...
      pj_custo: D.pj_custo,
      folha, aluguel, facilities, terceirizados, outros_cf, mkt,
  ...
  function gerarUpsides(D, dre, ise, setorCode) {
    ...
    // Concentração alta
    if (n(D.concentracao_pct) > 20 || D.concentracao_pct === undefined) {
    ...
    // Implantar contabilidade — sempre relevante se ICD baixo
    if(icd && icd.pct < 70) {
      ops.push({ titulo:'Implantar contabilidade gerencial mensal', descricao:'DRE mensal formal aumenta o ICD e torna o valuation mais defensável em due diligence.', ganho:0, ganho_label:'+ICD', tipo:'calculada' });
    }
