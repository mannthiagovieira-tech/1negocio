/** * Chat IA 1NEGÓCIO - Widget * Versao 2.8 - 2026-05-01 * Design: frosted glass, logo 1N identica a home * v2.8: tool calling valuation rápido + JWT logado + validação telefone BR */
(function() {
  'use strict';
  ...
  const STYLES = `
    ...
    #n1-chat-btn{position:fixed;bottom:24px;right:24px;width:56px;height:56px;background:transparent;border:2px solid #0aa85a;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:2147483646;transition:all .2s ease;box-shadow:0 0 12px #0aa85a,0 0 24px rgba(10,168,90,.35),inset 0 0 10px rgba(10,168,90,.06);overflow:hidden}
    #n1-chat-btn:hover{transform:scale(1.06);background:rgba(10,168,90,.12);box-shadow:0 0 20px #0aa85a,0 0 40px rgba(10,168,90,.5),inset 0 0 16px rgba(10,168,90,.1);border-color:#0aa85a}
    #n1-chat-btn.open{background:rgba(10,168,90,.15);border-color:#0aa85a;box-shadow:0 0 16px #0aa85a,0 0 32px rgba(10,168,90,.4)}
    #n1-chat-btn .n1-open-icon svg{width:24px;height:24px;filter:drop-shadow(0 0 6px #0aa85a)}
    #n1-chat-btn .n1-close-icon{display:none;color:#0aa85a;font-size:22px;font-family:'Cabinet Grotesk',sans-serif;font-weight:700;line-height:1;text-shadow:0 0 8px #0aa85a}
    ...
    #n1-chat-panel{position:fixed;bottom:96px;right:24px;width:370px;max-width:calc(100vw - 32px);height:560px;max-height:calc(100vh - 120px);background:rgba(255,255,255,.82);backdrop-filter:blur(18px) saturate(1.5);-webkit-backdrop-filter:blur(18px) saturate(1.5);border:1.5px solid rgba(10,168,90,.2);border-radius:24px;display:none;flex-direction:column;font-family:'Cabinet Grotesk',sans-serif;color:#0d2b1e;z-index:2147483645;box-shadow:0 8px 40px rgba(10,168,90,.12),0 2px 8px rgba(10,168,90,.08);overflow:hidden;animation:n1-slideUp .22s ease}
    ...
    #n1-chat-header{padding:13px 16px 11px;border-bottom:1px solid rgba(10,168,90,.15);display:flex;align-items:center;justify-content:space-between;background:rgba(247,247,255,.92);border-radius:24px 24px 0 0;flex-shrink:0}
    .n1-header-left{display:flex;align-items:center;gap:10px}
    .n1-logo-badge{display:flex;align-items:center;justify-content:center;width:38px;height:38px;background:#fff;border:1px solid rgba(13,43,30,.1);border-radius:9px;flex-shrink:0}
    .n1-logo-txt{font-family:'Syne',sans-serif;font-weight:800;font-size:16px;letter-spacing:-.04em;line-height:1;white-space:nowrap}
    .n1-logo-txt em{color:#0aa85a;font-style:normal}
    .n1-logo-txt span{color:#0d2b1e}
    .n1-header-info{display:flex;flex-direction:column;gap:1px}
    .n1-header-name{font-family:'Syne',sans-serif;font-weight:800;font-size:15px;color:#0d2b1e;letter-spacing:-.02em;line-height:1.2}
    .n1-header-name em{color:#0aa85a;font-style:normal}
    .n1-header-status{font-family:'Cabinet Grotesk',sans-serif;font-size:11px;color:#0aa85a;font-weight:500;display:flex;align-items:center;gap:5px}
    .n1-header-status::before{content:'';width:6px;height:6px;background:#0aa85a;border-radius:50%;animation:n1-blink 2.5s infinite}
    @keyframes n1-blink{0%,100%{opacity:1}50%{opacity:.35}}
    #n1-chat-close{background:rgba(10,168,90,.1);border:none;border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#0aa85a;font-size:16px;font-family:'Cabinet Grotesk',sans-serif;transition:background .15s;flex-shrink:0}
    #n1-chat-close:hover{background:rgba(10,168,90,.2)}
    #n1-chat-messages{flex:1;overflow-y:auto;padding:14px 12px;display:flex;flex-direction:column;gap:10px;scrollbar-width:thin;scrollbar-color:rgba(10,168,90,.2) transparent}
    #n1-chat-messages::-webkit-scrollbar{width:4px}
    #n1-chat-messages::-webkit-scrollbar-thumb{background:rgba(10,168,90,.2);border-radius:2px}
    ...
    .n1-msg.user{align-self:flex-end;background:#0aa85a;color:#fff;font-weight:500;border-radius:16px 16px 4px 16px}
    .n1-msg.bot{align-self:flex-start;background:rgba(255,255,255,.88);color:#0d2b1e;border-radius:4px 16px 16px 16px;border:1px solid rgba(10,168,90,.15);box-shadow:0 1px 4px rgba(10,168,90,.08)}
    .n1-msg.bot a{color:#0aa85a;text-decoration:underline}
    .n1-msg.bot strong{color:#0d2b1e;font-weight:700}
    .n1-typing{align-self:flex-start;padding:10px 14px;background:rgba(255,255,255,.88);border-radius:4px 16px 16px 16px;border:1px solid rgba(10,168,90,.15);display:flex;gap:4px;align-items:center}
    .n1-typing span{width:6px;height:6px;background:#0aa85a;border-radius:50%;animation:n1-dot 1.4s infinite;opacity:.4}
    ...
    .n1-quick-btn{background:rgba(255,255,255,.92);border:1.5px solid rgba(10,168,90,.4);color:#0aa85a;padding:6px 12px;font-family:'Cabinet Grotesk',sans-serif;font-size:12.5px;font-weight:600;cursor:pointer;transition:all .15s;border-radius:20px;line-height:1.3}
    .n1-quick-btn:hover{background:#0aa85a;color:#fff;border-color:#0aa85a}
    #n1-chat-input-area{border-top:1px solid rgba(10,168,90,.1);padding:10px 12px;display:flex;gap:8px;align-items:flex-end;background:rgba(247,247,255,.92);flex-shrink:0}
    #n1-chat-input{flex:1;background:rgba(255,255,255,.95);border:1.5px solid rgba(10,168,90,.2);color:#0d2b1e;padding:9px 13px;font-family:'Cabinet Grotesk',sans-serif;font-size:13.5px;font-weight:400;outline:none;resize:none;min-height:38px;max-height:100px;border-radius:20px;transition:border-color .15s}
    #n1-chat-input::placeholder{color:rgba(13,43,30,.35)}
    #n1-chat-input:focus{border-color:#0aa85a;box-shadow:0 0 0 3px rgba(10,168,90,.12)}
    #n1-chat-send{background:#0aa85a;color:#fff;border:none;width:36px;height:36px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:background .15s,transform .1s;margin-bottom:1px}
    ...
    .n1-form input{background:#fff;border:1.5px solid rgba(10,168,90,.25);color:#0d2b1e;padding:9px 12px;font-family:'Cabinet Grotesk',sans-serif;font-size:13px;outline:none;border-radius:12px;transition:border-color .15s;width:100%}
    .n1-form input::placeholder{color:rgba(13,43,30,.35)}
    .n1-form input:focus{border-color:#0aa85a}
    .n1-form button{background:#0aa85a;color:#fff;border:none;padding:10px 14px;font-family:'Cabinet Grotesk',sans-serif;font-weight:700;font-size:13px;cursor:pointer;border-radius:20px;transition:background .15s}
    .n1-form button:hover{background:#9061f9}
    .n1-form button.skip{background:transparent;border:1.5px solid rgba(10,168,90,.3);color:rgba(13,43,30,.45);font-weight:500}
    .n1-form button.skip:hover{background:rgba(10,168,90,.06);color:rgba(13,43,30,.65)}
    #n1-welcome-bubble{position:fixed;bottom:92px;right:24px;background:#fff;border-radius:16px 16px 4px 16px;padding:10px 32px 10px 14px;font-family:'Cabinet Grotesk',sans-serif;font-size:13.5px;font-weight:500;color:#0d2b1e;box-shadow:0 4px 20px rgba(10,168,90,.2);border:1px solid rgba(10,168,90,.3);z-index:2147483644;max-width:230px;line-height:1.4;cursor:pointer;animation:n1-bubbleIn .35s ease;display:none}
  ...
  const HTML = `
    <button id="n1-chat-btn" aria-label="Abrir chat 1NEGÓCIO" title="Fale com a 1NEGÓCIO">
      <span class="n1-open-icon"><svg viewBox="0 0 24 24" fill="none" stroke="#0aa85a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg></span>
  ...
  function startConversation() {
    ...
    // Limpar qualquer "Inicie a conversa" residual do sessionStorage (bug fix 02mai2026)
    state.messages = state.messages.filter(function(m) { return !(m.role === 'user' && m.content && m.content.startsWith('Inicie a conversa')); });
  ...
  function handleSend() {
    ...
    // Modo de captura de telefone — valida antes de enviar ao backend
    if (state.phoneCollectionMode && !state.lead.whatsapp) {
      const tel = validarTelefoneBR(text);
      if (!tel) {
        input.value = '';
        input.style.height = 'auto';
  ...
  async function sendToBackend() {
    try {
      const res = await fetch(API_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json', 'apikey': ANON_KEY, 'Authorization': 'Bearer ' + ANON_KEY }, body: JSON.stringify({
      ...
      // Remove qualquer "Inicie a conversa" residual do array (bug fix 02mai2026)
      state.messages = state.messages.filter(function(m) { return !(m.role === 'user' && m.content && m.content.startsWith('Inicie a conversa')); });
  ...
  function askForPhone() {
    state.phoneCollectionMode = true;
    const pn = state.lead.nome ? state.lead.nome.split(' ')[0] : '';
    const cumpr = pn ? pn + ', ' : '';
    const wrap = document.createElement('div');
    wrap.className = 'n1-msg bot';
    wrap.innerHTML = '<div>' + cumpr + 'se quiser continuar essa conversa com o nosso time no WhatsApp é só clicar aqui embaixo — levo o contexto da nossa conversa comigo.</div><div class="n1-quick" style="margin-top:10px"><button type="button" class="n1-quick-btn" id="n1-go-wpp">Continuar no WhatsApp</button><button type="button" class="n1-quick-btn" id="n1-stay-here" style="background:transparent">Agora não</button></div>';
    document.getElementById('n1-chat-messages').appendChild(wrap);
    scrollToBottom();
    wrap.querySelector('#n1-go-wpp').addEventListener('click', function() {
      wrap.remove();
      renderUserMessage('Quero continuar no WhatsApp');
      openWhatsApp();
      renderBotMessage('Perfeito! Te encontro lá.');
      // marca que já ofereceu pra não repetir
      state.leadCaptured = true;
    });
    wrap.querySelector('#n1-stay-here').addEventListener('click', function() {
      wrap.remove();
      renderBotMessage('Tranquilo! Sigo por aqui se precisar.');
      state.leadCaptured = true;
    });
  }
