/** * Chat IA 1Negocio - Widget * Versao 2.6 - 2026-04-17 * Design: frosted glass, Cabinet Grotesk, logo 1N identica a home * v2.6: botao neon roxo vazado (modelo A) + tema roxo completo */
(function() {
  'use strict';
  ...
  const STYLES = `
    ...
    #n1-chat-btn{position:fixed;bottom:24px;right:24px;width:56px;height:56px;background:transparent;border:2px solid #a78bfa;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:2147483646;transition:all .2s ease;box-shadow:0 0 12px #a78bfa,0 0 24px rgba(167,139,250,.35),inset 0 0 10px rgba(167,139,250,.06);overflow:hidden}
    #n1-chat-btn:hover{transform:scale(1.06);background:rgba(167,139,250,.12);box-shadow:0 0 20px #a78bfa,0 0 40px rgba(167,139,250,.5),inset 0 0 16px rgba(167,139,250,.1);border-color:#c4b5fd}
    #n1-chat-btn.open{background:rgba(167,139,250,.15);border-color:#c4b5fd;box-shadow:0 0 16px #a78bfa,0 0 32px rgba(167,139,250,.4)}
    #n1-chat-btn .n1-open-icon svg{width:24px;height:24px;filter:drop-shadow(0 0 6px #a78bfa)}
    #n1-chat-btn .n1-close-icon{display:none;color:#a78bfa;font-size:22px;font-family:'Cabinet Grotesk',sans-serif;font-weight:700;line-height:1;text-shadow:0 0 8px #a78bfa}
    ...
    .n1-logo-txt em{color:#a78bfa;font-style:normal}
    ...
    .n1-header-name em{color:#a78bfa;font-style:normal}
    .n1-header-status{font-family:'Cabinet Grotesk',sans-serif;font-size:11px;color:#a78bfa;font-weight:500;display:flex;align-items:center;gap:5px}
    .n1-header-status::before{content:'';width:6px;height:6px;background:#a78bfa;border-radius:50%;animation:n1-blink 2.5s infinite}
    @keyframes n1-blink{0%,100%{opacity:1}50%{opacity:.35}}
    #n1-chat-close{background:rgba(167,139,250,.1);border:none;border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#a78bfa;font-size:16px;font-family:'Cabinet Grotesk',sans-serif;transition:background .15s;flex-shrink:0}
    ...
    .n1-msg.user{align-self:flex-end;background:#a78bfa;color:#fff;font-weight:500;border-radius:16px 16px 4px 16px}
    ...
    .n1-msg.bot a{color:#a78bfa;text-decoration:underline}
    ...
    .n1-typing span{width:6px;height:6px;background:#a78bfa;border-radius:50%;animation:n1-dot 1.4s infinite;opacity:.4}
    ...
    .n1-quick-btn{background:rgba(255,255,255,.92);border:1.5px solid rgba(167,139,250,.4);color:#a78bfa;padding:6px 12px;font-family:'Cabinet Grotesk',sans-serif;font-size:12.5px;font-weight:600;cursor:pointer;transition:all .15s;border-radius:20px;line-height:1.3}
    .n1-quick-btn:hover{background:#a78bfa;color:#fff;border-color:#a78bfa}
    ...
    #n1-chat-input:focus{border-color:#a78bfa;box-shadow:0 0 0 3px rgba(167,139,250,.12)}
    #n1-chat-send{background:#a78bfa;color:#fff;border:none;width:36px;height:36px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:background .15s,transform .1s;margin-bottom:1px}
    ...
    .n1-form input:focus{border-color:#a78bfa}
    .n1-form button{background:#a78bfa;color:#fff;border:none;padding:10px 14px;font-family:'Cabinet Grotesk',sans-serif;font-weight:700;font-size:13px;cursor:pointer;border-radius:20px;transition:background .15s}
  ...
  const HTML = `
    <button id="n1-chat-btn" aria-label="Abrir chat 1Negocio" title="Fale com a 1Negocio">
      <span class="n1-open-icon"><svg viewBox="0 0 24 24" fill="none" stroke="#a78bfa" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg></span>
      <span class="n1-close-icon">&#215;</span>
      <span class="n1-pulse"></span>
    </button>
    <div id="n1-chat-panel" role="dialog" aria-label="Chat 1Negocio">
      <div id="n1-chat-header">
        <div class="n1-header-left">
          ...
          <div class="n1-header-info">
            <div class="n1-header-name"><em>1</em>Negocio</div>
      ...
      <div id="n1-chat-footer">1Negocio &middot; Diagnostico + avaliacao de empresas</div>
    </div>
  ...
  function openWhatsApp() {
    ...
    const msg = encodeURIComponent('Ola, vim do chat da 1Negocio.\nPerfil: ' + (state.perfil || 'nao identificado') + '\n\n' + resumo);
