// =====================================================
// SKILL AVALIADORA — 1Negócio v3.0
// Única fonte de cálculo. Laudos só leem calc_json.
// =====================================================
...
if (!window.AVALIADORA) { window.AVALIADORA = (() => {
  ...
  const p1  = (...vs) => { for (const v of vs) { const f = parseFloat(v); if (v !== undefined && v !== null && v !== '' && !isNaN(f) && f > 0) return f; } return 0; };
  ...
  function mapSetor(setor) {
    ...
    const mapa = {
      ...
      'consultoria':'servicos_b2b','agencia':'servicos_b2b','tecnologia':'servicos_b2b',
      'software':'servicos_b2b','ti':'servicos_b2b','contabilidade':'servicos_b2b',
      'advocacia':'servicos_b2b','saas':'servicos_b2b','b2b':'servicos_b2b',
    };
  ...
  function mapModelo(multi) {
    ...
    for (const m of ordem) { if (multi.includes(m)) return m; }
  ...
  // ─── 5. IMPOSTOS ─────────────────────────────────────────────────
  function calcImpostos(fatMensal, regime, setorCode, fatAnual) {
    ...
    if (regime === 'mei') {
      ...
      return { mensal: fixo, pct: fat > 0 ? fixo / fat * 100 : 0, regime: 'MEI', detalhes: 'Valor fixo mensal' };
    }
    ...
    if (regime === 'lucro_presumido' || regime === 'presumido') {
      ...
      return {
        ...
        detalhes: `PIS/COFINS 3,65% + IRPJ/CSLL s/ base ${tipoServico ? '32' : '8'}% (${pctTotal.toFixed(1)}% efetivo)`,
    ...
    if (regime === 'lucro_real') {
      ...
      const ro = 0; // sem RO aqui, usar zero conservador
      const irpj = ro > 0 ? ro * 0.15 + Math.max(0, ro - 20000) * 0.10 : 0;
      const csll = ro > 0 ? ro * 0.09 : 0;
      const total = pisCofins + irpj + csll;
      const pctTotal = fat > 0 ? total / fat * 100 : 0;
      return { mensal: total, pct: pctTotal, regime: 'Lucro Real', detalhes: `PIS/COFINS 9,25% s/ fat (${pctTotal.toFixed(1)}% efetivo)` };
    }
  ...
  function calcEncargosCLT(folhaBruta, regime) {
    ...
    const pctEncargos = (regime === 'mei') ? 0.11 : (regime === 'simples' || regime === 'simples_nacional') ? 0.08 : 0.358;
    const encargos = folhaBruta * pctEncargos;
    const provisoes = folhaBruta * 0.2314;
  ...
  function mapDados(dados) {
    ...
    // Normalizar regime
    const regimeRaw = (d.regime || d.regime_tributario || dados.regime_tributario || dados.regime || 'simples').toLowerCase().replace(/ /g, '_');
    let regime = regimeRaw;
    if (regime === 'simples_nacional') regime = 'simples';
    if (regime === 'presumido') regime = 'lucro_presumido';
    if (regime === 'real') regime = 'lucro_real';
    ...
    return {
      ...
      // Impostos pré-calculados (do diagnóstico)
      ...
      // Franquia
      royalty:   n(d.royalty_valor) > 0 ? n(d.royalty_valor) : n(d.royalty_pct) > 0 ? fat * n(d.royalty_pct) / 100 : 0,
      mkt_franq: n(d.mkt_franquia_valor) > 0 ? n(d.mkt_franquia_valor) : n(d.mkt_franquia_pct) > 0 ? fat * n(d.mkt_franquia_pct) / 100 : 0,

      // CMV
      cmv:           n(d.cmv_valor) > 0 ? n(d.cmv_valor) : n(d.cmv_pct) > 0 ? fat * n(d.cmv_pct) / 100 : 0,
      cmv_pct:       n(d.cmv_pct),
      cmv_informado: d.cmv_fonte === 'servico_puro' || d.cmv_fonte === 'informado' || d.cmv_fonte === 'informado_pct' || n(d.cmv_pct) > 0 || n(d.cmv_valor) > 0,
      ...
      processos_juridicos: d.processos_juridicos === 'sim' || d.processos_juridicos === true,
      recorrencia_pct: (() => {
      ...
      crescimento: (() => {
        ...
        if (['10a20', 'ate20', 'mais20', 'ate10', 'estavel'].includes(c)) return c;
  ...
  function calcDRE(D, setorCode) {
    const fat = D.fat;
    if (!fat) return { fat: 0, impostos: 0, taxas: 0, comissoes: 0, royalty: 0, mkt_franq: 0, recLiq: 0, cmv: 0, lb: 0, clt_folha: 0, encargos: 0, provisoes: 0, pj_custo: 0, folha: 0, aluguel: 0, facilities: 0, terceirizados: 0, sistemas: 0, outros_cf: 0, mkt: 0, ro: 0, prol: 0, antecipacao: 0, parcelas: 0, investimentos: 0, potencial: 0, margem_pct: 0, estimados: {} };
    ...
    if (D.impostos_precalc > 0) {
      ...
      impostos = calcImpostos(fat, D.regime, setorCode, D.fatAnual).mensal;
    }
    ...
    const outros_cf = (D.outros_cf > 0) ? D.outros_cf : (cf_inf ? 0 : Math.round(fat * bench.cf / 100));

    // Marketing — nunca estimamos
    const mkt = D.mkt > 0 ? D.mkt : 0;

    // DRE
    const recLiq = fat - impostos - D.taxas - D.comissoes - D.royalty - D.mkt_franq;
    const lb = recLiq - cmv;
    const ro = lb - folha - aluguel - D.facilities - D.terceirizados - D.sistemas - outros_cf - mkt;
    const potencial = ro - D.prolabore - D.antecipacao - D.parcelas - D.investimentos;

    const estimados = {
      cmv:       !cmv_inf && cmv > 0,
      folha:     !folha_inf && folha > 0,
      aluguel:   !aluguel_inf && aluguel > 0,
      outros_cf: !cf_inf && outros_cf > 0,
    };

    return {
      fat, impostos, taxas: D.taxas, comissoes: D.comissoes,
      royalty: D.royalty, mkt_franq: D.mkt_franq,
      recLiq, cmv, lb,
      ...
      pj_custo: D.pj_custo,
      folha, aluguel,
      facilities: D.facilities, terceirizados: D.terceirizados,
      sistemas: D.sistemas, outros_cf, mkt,
      ro, prol: D.prolabore, antecipacao: D.antecipacao,
  ...
  function calcISE(D, dre, bal) {
    ...
    // P1 — Dependência
    const p1v = D.dependencia === 'total' ? 1 : D.dependencia === 'nenhuma' ? 8 : 5;

    // P2 — Comercial (recorrência + base de clientes)
    ...
    // P7 — Dívida (parcelas mensais vs RO anual)
    ...
    // Trava dupla: 2+ pilares críticos abaixo de 3 → limita a 40
    const criticos = [p1v, p2, p3, p4, p6].filter(v => v < 3).length;
    const total = criticos >= 2 ? Math.min(bruto, 40) : Math.min(100, Math.max(0, bruto));

    const cls = total >= 85 ? 'Estruturado' : total >= 70 ? 'Sólido' : total >= 50 ? 'Operacional' : total >= 35 ? 'Dependente' : 'Embrionário';

    return { total, cls, dep: p1v, com: p2, fin: p3, ges: p4, mar: p5, bal: p6, div: p7, ris: p8, conc: p9, esc: p10 };
  }

  // ─── 11. FATOR 1N ────────────────────────────────────────────────
  function calcFator(D, iseTotal, setorCode) {
    ...
    // Leitura segura: número direto ou .mid se objeto legado
    const rawMb = mb[modeloCode] || mb['default'] || 4;
    const multiploBase = typeof rawMb === 'object' ? n(rawMb.mid) : n(rawMb);
    const modificador  = n(ms[setorCode] || 0);

    // Fator ISE
    let fatorIse = 1.0, iseNome = 'Operacional';
    if (fi.length > 0) {
      const f = fi.find(x => iseTotal >= x.min && iseTotal <= x.max);
      if (f) { fatorIse = n(f.fator); iseNome = f.nome; }
    } else {
      if (iseTotal >= 85)      { fatorIse = 1.30; iseNome = 'Estruturado'; }
      else if (iseTotal >= 70) { fatorIse = 1.15; iseNome = 'Consolidado'; }
      else if (iseTotal >= 50) { fatorIse = 1.00; iseNome = 'Operacional'; }
      else if (iseTotal >= 35) { fatorIse = 0.85; iseNome = 'Dependente'; }
      else                     { fatorIse = 0.70; iseNome = 'Embrionário'; }
    }

    const fMin = n(lim.fator_min || 1.5);
    const fMax = n(lim.fator_max || 6.0);
    const fator = Math.max(fMin, Math.min(fMax, (multiploBase + modificador) * fatorIse));

    return {
      fator, modeloCode, setorCode, multiploBase, modificador,
      fatorIse, iseNome,
  ...
  function calcAtratividade(D, dre, ise, setorCode) {
    ...
    // P1 — Solidez ISE (0–10 proporcional)
    ...
    // Retorna score e TODOS os sub-pilares com nomes explícitos
    return { score: score || 5, lbl, sol: p1, set: p2, rec: p3, ind: p4, cre: p5, mar: p6 };
  }

  // ─── 13. ICD ─────────────────────────────────────────────────────
  // Índice de Confiabilidade dos Dados
  // Lógica: campos obrigatórios (peso 3), desejáveis (peso 1)
  // Gestão NÃO conta automaticamente — só conta se preenchido explicitamente
  function calcICD(D, dre) {
    const d = D._raw || {};

    // OBRIGATÓRIOS (peso 3 cada): bloqueiam avanço no diagnóstico
    const obrig = {
      fat_mensal:  D.fat > 0,
      regime:      !!D.regime,
      cmv:         D.cmv_informado,
    ...
    // DESEJÁVEIS (peso 1 cada): penaliza ausência
    const desej = {
      folha_clt:    D.clt_folha > 0,
      aluguel:      D.aluguel > 0,
      mkt:          n(d.mkt_valor) > 0,
      outros_cf:    n(d.custo_outros) > 0,
      prolabore:    D.prolabore > 0,
      recorrencia:  d.recorrencia_pct !== undefined && d.recorrencia_pct !== null && d.recorrencia_pct !== 'nao',
      ...
      anos:         n(d.anos_existencia) > 0,
      // Gestão: CONTA APENAS se preenchido diferente do default
      processos:    d.processos && d.processos !== 'parcial',
      dependencia:  d.dependencia && d.dependencia !== 'parcial',
    };

    const pesoObrig = 3;
    const pesoDesej = 1;

    const maxPontos  = Object.keys(obrig).length * pesoObrig + Object.keys(desej).length * pesoDesej;
    const pontosObtig = Object.values(obrig).filter(Boolean).length * pesoObrig;
    const pontosDesej = Object.values(desej).filter(Boolean).length * pesoDesej;
    const total = pontosObtig + pontosDesej;

    const respondidos    = [...Object.keys(obrig).filter(k => obrig[k]), ...Object.keys(desej).filter(k => desej[k])];
    const naoRespondidos = [...Object.keys(obrig).filter(k => !obrig[k]), ...Object.keys(desej).filter(k => !desej[k])];
    ...
    const labelMap = {
      ...
      anos: 'Anos de operação', processos: 'Processos documentados',
      dependencia: 'Dependência do sócio',
    };

    return {
      pct: Math.round(total / maxPontos * 100),
      respondidos:     respondidos.map(k => labelMap[k] || k),
      nao_respondidos: naoRespondidos.map(k => labelMap[k] || k),
      total_campos:    Object.keys(obrig).length + Object.keys(desej).length,
  ...
  function calcRegimes(D, dre, setorCode) {
    const fat    = dre.fat || 0;
    const fatAnual = D.fatAnual || fat * 12;
    const ro     = dre.ro || 0;
    const regimeAtual = D.regime || 'simples';
    const resultados = [];
    ...
    if (fatAnual <= 81000) {
      ...
      resultados.push({ regime: 'MEI', elegivel: false, motivo: 'Faturamento acima de R$ 81k/ano', imposto_mensal: 0, pct: 0 });
    }

    // Simples Nacional
    if (fatAnual <= 4800000) {
      const imp = calcImpostos(fat, 'simples', setorCode, fatAnual);
      resultados.push({ regime: 'Simples Nacional', elegivel: true, imposto_mensal: imp.mensal, pct: imp.pct, detalhes: imp.detalhes });
    } else {
      resultados.push({ regime: 'Simples Nacional', elegivel: false, motivo: 'Faturamento acima de R$ 4,8M/ano', imposto_mensal: 0, pct: 0 });
    }

    // Lucro Presumido
    const impPresumido = calcImpostos(fat, 'lucro_presumido', setorCode, fatAnual);
    ...
    const regimeAtualLabel = regimeNormMap[regimeAtual] || 'Simples Nacional';
    const atual = resultados.find(r => r.regime === regimeAtualLabel) || elegiveis[0];
    const impostoAtual = atual ? atual.imposto_mensal : 0;
    const economia     = Math.max(0, impostoAtual - otimo.imposto_mensal);

    return {
      regime_atual:        regimeAtual,
      regime_atual_label:  regimeAtualLabel,
      ...
      economia_mensal:     Math.round(economia),
      economia_anual:      Math.round(economia * 12),
      tem_oportunidade:    fat > 0 && economia > fat * 0.01,
    };
  }
  ...
  function gerarUpsides(D, dre, ise, setorCode, regimes) {
    ...
    // 1. Tributário — sempre primeiro
    if (regimes && regimes.tem_oportunidade && regimes.economia_mensal > 0) {
      ops.push({
        titulo: 'Migrar para regime tributário mais eficiente',
        descricao: `Regime atual: ${regimes.regime_atual_label}. Migração para ${regimes.regime_otimo} pode reduzir a carga tributária em ${brl(regimes.economia_mensal)}/mês (${brl(regimes.economia_anual)}/ano). Validar com contador.`,
    ...
    // 2. CMV acima do benchmark
    const cmvPct = fat > 0 ? pct(dre.cmv, fat) : 0;
    if (cmvPct > bench.cmv + 3) {
      const ganho = Math.round(fat * (cmvPct - bench.cmv) / 100 * 0.4);
      if (ganho > 0) ops.push({ titulo: 'Reduzir custo de produção ao benchmark setorial', descricao: `CMV atual em ${cmvPct.toFixed(0)}% vs benchmark de ${bench.cmv}% para o setor. Renegociação de fornecedores pode liberar resultado imediato.`, ganho, ganho_label: `+${brl(ganho)}/mês`, tipo: 'calculada' });
    }

    // 3. Folha acima do benchmark
    ...
    if (folhaPct > 0 && folhaPct > benchInd.folha_pct * 1.2) {
      ...
      if (ganho > 0) ops.push({ titulo: 'Otimizar estrutura de equipe', descricao: `Folha em ${folhaPct.toFixed(0)}% do faturamento vs benchmark de ${benchInd.folha_pct}%. Revisão de estrutura pode liberar ${brl(ganho)}/mês.`, ganho, ganho_label: `+${brl(ganho)}/mês`, tipo: 'calculada' });
    }

    // 4. Aluguel acima do benchmark
    ...
    if (dre.aluguel > 0 && aluPct > benchInd.aluguel_pct * 1.3) {
      ...
      if (ganho > 0) ops.push({ titulo: 'Renegociar contrato de aluguel', descricao: `Aluguel em ${aluPct.toFixed(0)}% vs benchmark de ${benchInd.aluguel_pct}%. Renegociação pode liberar ${brl(ganho)}/mês.`, ganho, ganho_label: `+${brl(ganho)}/mês`, tipo: 'calculada' });
    }

    // 5. Recorrência baixa
    const rec = n(D.recorrencia_pct);
    if (rec < 30) {
      const ganho = Math.round(fat * 0.10);
      ops.push({ titulo: 'Converter clientes avulsos em recorrentes', descricao: `Recorrência atual de ${rec}%. Estruturar planos mensais para 20% da base pode adicionar ${brl(ganho)}/mês de receita previsível e elevar o múltiplo.`, ganho, ganho_label: `+${brl(ganho)}/mês`, tipo: 'calculada' });
    }

    // 6. Marketing abaixo do benchmark
    ...
    if (mktPct < mktBench * 0.5 && fat > 50000) {
      ...
      ops.push({ titulo: 'Estruturar canal de marketing digital', descricao: `Marketing atual em ${mktPct.toFixed(1)}% vs benchmark de ${mktBench}%. Investindo ${brl(investimento)}/mês com ROI 3x, potencial de +${brl(retorno)}/mês.`, ganho: retorno - investimento, ganho_label: `+${brl(retorno - investimento)}/mês líquido`, tipo: 'calculada' });
    }

    // 7. Fixas por setor
    const fixasPorSetor = {
      alimentacao: [
        { titulo: 'Expandir canal de delivery', descricao: 'Estruturar delivery próprio ou ampliar parceria com iFood/Rappi.', ganho: Math.round(fat * 0.10) },
        { titulo: 'Criar menu executivo / happy hour', descricao: 'Operação em horários ociosos com equipe existente.', ganho: Math.round(fat * 0.06) },
        { titulo: 'Eventos privados e locação do espaço', descricao: 'Receita complementar com margem elevada em dias de menor movimento.', ganho: Math.round(fat * 0.05) },
      ],
      saude: [
        { titulo: 'Estruturar pacotes e planos de procedimentos', descricao: 'Planos com pagamento recorrente aumentam previsibilidade.', ganho: Math.round(fat * 0.12) },
        { titulo: 'Expandir horário de atendimento', descricao: 'Turnos adicionais com equipe existente sem custo fixo relevante.', ganho: Math.round(fat * 0.08) },
      ...
      educacao: [
        { titulo: 'Criar turmas online ou híbridas', descricao: 'Escala sem crescimento proporcional de custos fixos.', ganho: Math.round(fat * 0.15) },
    ...
    const default_ops = [
      { titulo: 'Expandir base de clientes ativos', descricao: 'Crescer 20% em clientes mantendo ticket médio via outreach estruturado.', ganho: Math.round(fat * 0.08) },
      { titulo: 'Estruturar funil de upsell para base atual', descricao: 'Oferecer produtos ou serviços complementares. Crescimento sem custo de aquisição.', ganho: Math.round(fat * 0.07) },
    ...
    const fixas = [...(fixasPorSetor[setorCode] || []), ...default_ops];
    ...
    // Ordenar: tributário primeiro, depois por ganho decrescente
    ops.sort((a, b) => {
    ...
    return { ops: top10, total: top10.reduce((s, o) => s + (o.ganho || 0), 0) };
  }

  // ─── 16. MONTAR calc_json ─────────────────────────────────────────
  function montarCalcJson(D, dre, bal, ise, fatorObj, atr, icd, upsides, regimes) {
    ...
    return {
      ...
      // Atratividade — TODOS os sub-pilares explícitos
      atr_score:        atr.score,
      atr_lbl:          atr.lbl,
      atr_sol:          atr.sol,
      atr_set:          atr.set,
      ...
      // Tributário — objeto completo
      analise_regimes:  regimes,

      // Benchmarks para os laudos renderizarem
      bench_dre:        bench,
      bench_ind:        benchInd,
  ...
  async function salvarCalcJson(negocioId, calcJson) {
    ...
    try {
      ...
      const r = await fetch(SB_URL + '/rest/v1/laudos_completos', {
        method: 'POST',
      ...
      if (r.ok) {
        ...
        console.error('[AVALIADORA] Erro upsert:', r.status, err);
      }
    } catch (e) {
  ...
  async function avaliar(dadosBrutos) {
    ...
    const calcJson = montarCalcJson(D, dre, bal, ise, fatorObj, atr, icd, upsides, regimes);

    console.log('[AVALIADORA] Resultado:', {
      ise: ise.total, fator: fatorObj.fator,
      ro: dre.ro, valor: calcJson.valor_venda,
      atr: atr.score, icd: icd.pct,
      atr_sub: { sol: atr.sol, set: atr.set, rec: atr.rec, ind: atr.ind, cre: atr.cre, mar: atr.mar },
    });
